...
void gdaxWebSocketHandler::onDisconnected() {
    ...
    if ( mWS != NULL ) { QObject::disconnect(mWS,&QWebSocket::textMessageReceived, this, &gdaxWebSocketHandler::onTextMessageReceived);QObject::disconnect(mWS,&QWebSocket::connected, this, &gdaxWebSocketHandler::onConnected); QObject::disconnect(mWS,&QWebSocket::disconnected, this, &gdaxWebSocketHandler::onDisconnected); delete mWS; }
    mWS = new QWebSocket();
    QObject::connect(mWS, &QWebSocket::connected, this, &gdaxWebSocketHandler::onConnected);
    mWS->open(QUrl(mWSHost));
    if ( mWS->errorString() != "Unknown error") {
