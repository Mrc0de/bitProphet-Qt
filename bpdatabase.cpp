...
void bpDatabase::getGdaxPriceHistoryFrom(QString coin,QString dateTimeFrom, int maxHours, QList<QString> *priceList,QList<QString> *askList,QList<QString> *bidList,QList<QString> *tsList) {
    ...
    //2018-03-22 02:26:57
    {
        ...
        if (!Db.open()) {
           ...
           //say("Database: connection ok.");
            QSqlQuery query;
            QString sayMe("select * from gdaxPriceHistory WHERE coin='"+ coin +"' AND id in (select id from gdaxPriceHistory WHERE coin='"+ coin +"' AND ts >= datetime('"+dateTimeFrom+"','-"+QString().setNum(maxHours)+" hours') ORDER BY ts DESC) AND id % 10 = 0 ORDER BY ts ASC");
            say(sayMe);
            query.prepare("select * from gdaxPriceHistory WHERE coin='"+ coin +"' AND id in (select id from gdaxPriceHistory WHERE coin='"+ coin +"' AND ts >= datetime('"+dateTimeFrom+"','-"+QString().setNum(maxHours)+" hours') ORDER BY ts DESC) AND id % 10 = 0 ORDER BY ts ASC"); //First(Past) to Last(current)
            if (query.exec()) {
                int y=0;
               while (query.next()) {
                  int pVal = query.record().indexOf("price");
